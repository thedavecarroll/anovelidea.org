...
test.describe('Responsive Layout', () => {
  test('no horizontal overflow on mobile', async ({ page, baseURL }) => {
    ...
    for (const path of allPages) {
      ...
      if (overflow > 0) {
        overflowPages.push({ url: path, overflow });
      }
    }
  ...
  test('content stays within site wrapper on mobile', async ({ page, baseURL }) => {
    await page.setViewportSize({
      width: MOBILE_VIEWPORT.viewport.width,
      height: MOBILE_VIEWPORT.viewport.height,
    });
    ...
    const pagesToCheck = ['/', '/poetry/', '/fiction/', '/about/'];
    const violations: { url: string; elements: string[] }[] = [];

    for (const path of pagesToCheck) {
      await page.goto(`${baseURL}${path}`, { waitUntil: 'load' });

      // Find elements that extend beyond the site wrapper
      const overflowingElements = await page.evaluate(() => {
        ...
        const checkSelectors = '.site-wrapper > *, .main-content *, .prose *';
      ...
      if (overflowingElements.length > 0) {
        violations.push({ url: path, elements: overflowingElements });
      }
    }
